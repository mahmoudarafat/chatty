    ...
    if (messenger != 0) {
        ...
        $.ajax({
            ...
            success: (data) => {
                ...
                $(".messenger-infoView .info-id").html(data.fetch.client_id);
                $(".messenger-infoView .info-country").html(data.fetch.client.country_name);
                $(".messenger-infoView .info-mobile").html(data.fetch.mobile);

                $(".messenger-infoView .info-opened-tasks").html(data.fetch.client.opened_tasks_count);
                $(".messenger-infoView .support_code_tasks").attr('href', 'https://is2host.com/albadr-tasks/#/app/tasks/AllTasks?support_code=' + data.fetch.client_id);
 ...
 *-------------------------------------------------------------
 */
function sendMessage(blob = null) {
    console.log('entered')
    temporaryMsgId += 1;
    let tempID = `temp_${temporaryMsgId}`;
    let hasFile = !!$(".upload-attachment").val();
    let inputValue = $.trim(messageInput.val());

    console.log({blob})

    if(blob != null){
        inputValue = 'voice message';
    }

    console.log({tempID, hasFile, inputValue})
    ...
    if (inputValue.length > 0 || hasFile) {
        ...
        if (blob != null) {
            formData.append("audio_data", blob, 'test.wav');
        }
        for (var i=0; i<formData.length; i++)
        {
            console.log("formDataAttr: ",  formData[i].name, formData[i].value);
        }
        $.ajax({
...
// Listen to messages, and append if data received
channel.bind("contact-transform-to-agent", function (data) {
    if ( data.to_id == auth_id) {
       getContacts(true);
    }
});
...
function getContacts($updateTrigger=false) {
    console.log('getting contacts>>>>>>');
    if ( $updateTrigger || ( !contactsLoading && !noMoreContacts) ){
